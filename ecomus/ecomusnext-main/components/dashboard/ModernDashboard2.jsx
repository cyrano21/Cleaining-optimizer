...
const ModernDashboard = ({ role = 'client', user = {} }) => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const productsEndpoint = role === 'vendor' && user.storeId 
        ? `/api/products?storeId=${user.storeId}&limit=12&sort=createdAt&order=desc`
        : '/api/products?limit=12&sort=createdAt&order=desc'
      ...
      <div className="relative">
        ...
        {showActions && (
          <div className="absolute top-2 left-2 flex space-x-2">
            <button className="bg-white/90 hover:bg-white p-1 rounded-full">
  ...
  const renderOverview = () => (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          title="Revenus"
          value={`${Math.round((stats?.overview?.revenue || 0) * 100) / 100}€`}
          change="+23% ce mois"
          trend="up"
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="h-64 bg-gray-50 rounded-lg flex items-center justify-center">
          <p className="text-gray-500">Graphique d'optimisation Cloudinary</p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-8">
      ...
      {activeTab === 'analytics' && (role === 'admin' || role === 'vendor') && renderAnalytics()}
    </div>
  )
}
