...
import { AlertCircle, Shield, Store, User, ArrowRight } from "lucide-react";
import { motion } from "framer-motion";

/**
 * Dashboard Unifié - Point d'entrée adaptatif
 * Redirige intelligemment vers le bon dashboard selon le rôle utilisateur
 */
...
export default function UnifiedDashboard() {
  ...
  // Fallback pour rôles non reconnus ou erreurs
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-red-50 via-white to-orange-50">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="text-center space-y-4 p-8 bg-white rounded-2xl shadow-xl max-w-md mx-auto"
      >
        ...
        <p className="text-gray-600">
          Votre rôle utilisateur ne permet pas d'accéder à cette section.
        </p>
        <p className="text-sm text-gray-500">
          Rôle détecté : {session?.user?.role || 'Non défini'}
        </p>
      </motion.div>
    </div>
  );
